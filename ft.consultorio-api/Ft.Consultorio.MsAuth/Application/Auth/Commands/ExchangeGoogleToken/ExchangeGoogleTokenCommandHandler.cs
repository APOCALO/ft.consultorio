...
{
    internal sealed class ExchangeGoogleTokenCommandHandler
        : ApiBaseHandler<ExchangeGoogleTokenCommand, AuthTokenResponseDTO>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IUserRepository _userRepository;
        private readonly IGoogleIdTokenValidator _googleIdTokenValidator;
        private readonly IGoogleBirthDateResolver _googleBirthDateResolver;
        ...
        public ExchangeGoogleTokenCommandHandler(
            IUnitOfWork unitOfWork,
            ILogger<ExchangeGoogleTokenCommandHandler> logger,
            IUserRepository userRepository,
            IGoogleIdTokenValidator googleIdTokenValidator,
            IGoogleBirthDateResolver googleBirthDateResolver,
            ...
            : base(logger)
        {
            ...
            _googleIdTokenValidator = googleIdTokenValidator ?? throw new ArgumentNullException(nameof(googleIdTokenValidator));
            _googleBirthDateResolver = googleBirthDateResolver ?? throw new ArgumentNullException(nameof(googleBirthDateResolver));
        ...
        protected override async Task<ErrorOr<ApiResponse<AuthTokenResponseDTO>>> HandleRequest(
            ExchangeGoogleTokenCommand request,
            CancellationToken cancellationToken)
        {
            GoogleTokenPayload payload;
            try
            {
                payload = await _googleIdTokenValidator.ValidateAsync(request.IdToken, cancellationToken);
            }
            catch (InvalidJwtException ex)
            {
                _logger.LogWarning(ex, "Google id_token validation failed.");
                return Error.Unauthorized("Auth.Google.InvalidToken", "Google token is invalid or expired.");
            }
            catch (InvalidOperationException ex)
            {
                _logger.LogError(ex, "Google auth configuration error.");
                return Error.Unexpected(description: "Authentication service is misconfigured.");
            }

            if (!payload.EmailVerified)
            {
                return Error.Unauthorized("Auth.Google.EmailUnverified", "Google email is not verified.");
            }

            if (string.IsNullOrWhiteSpace(payload.Email))
            {
                return Error.Unauthorized("Auth.Google.EmailMissing", "Google token does not include an email.");
            }

            var normalizedEmail = payload.Email.Trim().ToLowerInvariant();
            var authUserId = $"google:{payload.Subject}";
            var resolvedBirthDate = await _googleBirthDateResolver.TryResolveAsync(
            ...
            if (user is null)
            {
                ...
                {
                    return Error.Conflict("Auth.Google.Email", "Email is already registered with another provider.");
                }

                var userId = Guid.NewGuid();
                var resolvedFirstName = payload.GivenName
                    ?? payload.FullName?.Split(' ', StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()
                    ?? "User";
                var resolvedLastName = payload.FamilyName
                    ?? payload.FullName?.Split(' ', StringSplitOptions.RemoveEmptyEntries).Skip(1).FirstOrDefault()
                    ?? "User";
            ...
            else
            {
                if (!user.IsActive)
                {
                    return Error.Forbidden("Auth.Google.Inactive", "User is inactive.");
                }

                var updated = false;

                if (!string.IsNullOrWhiteSpace(payload.GivenName) && payload.GivenName != user.FirstName)
                {
                    user.UpdateFirstName(payload.GivenName);
                    updated = true;
                }

                if (!string.IsNullOrWhiteSpace(payload.FamilyName) && payload.FamilyName != user.LastName)
                {
                    user.UpdateLastName(payload.FamilyName);
                    updated = true;
                }

                if (string.IsNullOrWhiteSpace(user.AvatarUrl) && !string.IsNullOrWhiteSpace(payload.PictureUrl))
                {
                    user.UpdateAvatarUrl(payload.PictureUrl);
                    updated = true;
                }

                if (!user.BirthDate.HasValue && resolvedBirthDate.HasValue)
                {
                    user.UpdateBirthDate(resolvedBirthDate);
                    updated = true;
                }
